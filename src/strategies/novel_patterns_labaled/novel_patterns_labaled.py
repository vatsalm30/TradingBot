...
class NovelPatternsLabaledStrategy(Strategy):
    ...
    def generate_trade(self, data: Dict[str, pd.DataFrame]):
        if data["5m"]["timestamp"].iloc[-1] != data["5rm"]["timestamp"].iloc[-1]:
        ...
        trendline_slope = trend_line.fit_trendlines_single(data.tail(14), 14)

        trade_data = pd.DataFrame(columns=["adx", "ema_24_100_diff", "support_trendline_slope", "resist_trendline_slope", "ema50_slope", "atr", "rsi", "return"])
        trade_data["adx"] = adx(data.tail(14), 14).tail(1)
        ...
        trade_data["support_trendline_slope"] = pd.Series([trendline_slope[0][0]])
        trade_data["resist_trendline_slope"] = pd.Series([trendline_slope[1][0]])
        trade_data["ema50_slope"] = moving_average_slope(data.tail(50), 50, exponential_moving_average).tail(1)
        trade_data["atr"] = average_true_range(data.tail(14), 14).tail(1)
        trade_data["rsi"] = rsi(data.tail(14), 14).tail(1)
