...
from utils.confidence_to_color import confidence_to_color
...
class PIPPatternMiner:
    ...
    def predict(self, pips_y: list):
        ...
        confidence = 1.0 - min(best_dist / 2.0, 1.0)

        # -1 is short, 0 is no, 1 is long

        if best_clust in self._selected_long:
            return 1.0, best_clust, confidence
        elif best_clust in self._selected_short:
            return -1.0, best_clust, confidence
        else:
            return 0.0, best_clust, confidence
    ...
    def show_cluster(self, cluster_id: int, data:pd.DataFrame):
        plt.style.use('dark_background')

        for index in range(len(data)):
            x = data['close'].iloc[len(data)-24 - index:len(data) - index].to_numpy()
            ...
            _, clust, confidence = self.predict(pips_y)
            norm_y = (np.array(pips_y) - np.mean(pips_y)) / np.std(pips_y)


            pips_x = [i for i in range(len(pips_y))]

            if clust == cluster_id:
                plt.plot(pips_x, norm_y, color=confidence_to_color(confidence), linewidth=0.1)

        plt.plot(pips_x, self._cluster_centers[cluster_id], color="red")

        plt.show()

    def run_test(self, arr, reps=100):
        self.train(arr, n_reps=reps)
        actual_martin = self.get_fit_martin()
        perm_martins = self.get_permutation_martins()
        ...
        ax.set_title("Permutation's Martin Ratio ETH-USDT 1H 2022-2024")
